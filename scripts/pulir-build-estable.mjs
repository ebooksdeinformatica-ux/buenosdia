...
for(const post of posts){
 ...
 if(!html.includes('class="evidence-check"')){
   ...
   const block=`<section class="evidence-check"><h2>Cómo comprobar si estás avanzando</h2><p>Antes de modificar “${t}”, anotá el punto de partida y elegí una sola señal que puedas observar. El foco recomendado es ${esc(p.focus)}. La secuencia más útil es ${esc(p.steps)}. Trabajá con una versión pequeña durante varios días para evitar que el entusiasmo inicial o un mal día distorsionen la evaluación.</p><p>Al revisar el resultado, compará ${esc(p.measure)}. También controlá ${esc(p.risks)}. Si el cambio aporta valor y puede repetirse, mantenelo; si depende de condiciones perfectas, reducí su tamaño. En temas de salud, lesiones, seguridad o dinero, verificá la información y pedí orientación cuando una guía general no alcance.</p></section>`;
...
console.log(`Redacción pulida en ${changed} publicaciones.`);
