...
for (const [paragraph, owners] of shared) {
  if (owners.length >= 20) errors.push(`Párrafo repetido en ${owners.length} publicaciones: "${paragraph.slice(0, 110)}..."`);
  else if (owners.length >= 8) warnings.push(`Párrafo repetido en ${owners.length} publicaciones: "${paragraph.slice(0, 110)}..."`);
}

fs.mkdirSync('data', { recursive: true });
fs.writeFileSync('data/auditoria-publicaciones.json', `${JSON.stringify({ generatedAt:new Date().toISOString(), auditedFiles:files.length, minimumWords:MIN_WORDS, errors, warnings }, null, 2)}\n`);

console.log(`Auditadas ${files.length} publicaciones. Errores: ${errors.length}. Advertencias: ${warnings.length}.`);
for (const error of errors.slice(0,100)) console.error(`ERROR: ${error}`);
console.warn('Modo diagnóstico de rama: el informe se guarda aunque existan errores.');
