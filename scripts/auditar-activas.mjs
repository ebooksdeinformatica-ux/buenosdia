...
const active=posts.map(p=>{const file=p.url.replace(/^\//,'');if(!fs.existsSync(file))fail('Falta archivo: '+file);const html=fs.existsSync(file)?fs.readFileSync(file,'utf8'):'';const title=(html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i)||[])[1]||p.title;const visible=text(html);const intro=[...html.matchAll(/<p[^>]*>([\s\S]*?)<\/p>/gi)].slice(0,3).map(m=>text(m[1])).join(' ');return{file,title,intro,body:visible,tags:p.tags||[]}});
for(const p of active){if(p.title.length<35)fail(p.file+': título débil');if(/en este artículo vamos a ver|hoy vamos a hablar|en esta guía veremos/i.test(p.intro))fail(p.file+': arranque de plantilla');if(internal.test(p.body))fail(p.file+': contiene cocina interna visible');if(p.tags.length<3||p.tags.length>7)fail(p.file+': tags deben ser 3 a 7');for(const t of p.tags){if(bad.has(clean(t)))fail(p.file+': tag basura '+t)}}
