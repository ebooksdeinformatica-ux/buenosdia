...
const active=posts.map(p=>{const file=p.url.replace(/^\//,'');if(!fs.existsSync(file))fail('Falta archivo: '+file);const html=fs.existsSync(file)?fs.readFileSync(file,'utf8'):'';const title=(html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i)||[])[1]||p.title;const body=visibleText(html);const intro=[...html.matchAll(/<p[^>]*>([\s\S]*?)<\/p>/gi)].slice(0,3).map(m=>visibleText(m[1])).join(' ');return{file,html,title,intro,body,tags:p.tags||[],format:p.format||''}});
for(const p of active){if(p.title.length<35)fail(p.file+': título débil');if(/en este artículo vamos a ver|hoy vamos a hablar|en esta guía veremos/i.test(p.intro))fail(p.file+': arranque de plantilla');if(internal.test(p.body))fail(p.file+': contiene cocina interna visible');if(scaffolding.test(p.body))fail(p.file+': muestra andamiaje creativo o fuente interna');if(!/rel=["']apple-touch-icon["']/i.test(p.html))fail(p.file+': falta Apple Touch Icon');const wc=wordCount(p.body);if(wc<750)fail(p.file+': publicación demasiado corta ('+wc+' palabras limpias, mínimo 750)');const avg=avgWordsPerSentence(p.body);if(avg<10)fail(p.file+': promedio de palabras por frase muy bajo ('+avg.toFixed(2)+')');const longBold=strongTooLong(p.html);if(longBold.length)fail(p.file+': negrita demasiado larga: '+longBold[0].slice(0,90));if(active.length>2&&countInternalLinks(p.html)<4)fail(p.file+': pocos enlaces internos contextuales');if(p.tags.length<3||p.tags.length>7)fail(p.file+': tags deben ser 3 a 7');for(const t of p.tags){if(bad.has(clean(t)))fail(p.file+': tag basura '+t)}}
