...
const posts = files.map(file => {
  ...
  const visualTags = getVisibleTags(html).map(normalizeTag).filter(Boolean);
  const tags = [...new Set([...metaTags, ...visualTags])].slice(0, 12);
...
for (const post of posts) {
  ...
  const relatedSectionRegex = /<section class="card"[^>]*>\s*<h2[^>]*id="interlinks-post"[^>]*>Seguí leyendo<\/h2>[\s\S]*?<\/section>/i;
  ...
  updatedHtml = patchPostHtml(updatedHtml, post.tags);
  fs.writeFileSync(post.fullPath, updatedHtml, 'utf8');
}

renderTagPages(tagMap, posts);
...
function getVisibleTags(html = '') {
  const tagsSections = [...html.matchAll(/<section[^>]*(?:id=\"tags-post\"|class=\"[^\"]*tags-box[^\"]*\"|class=\"[^\"]*card tags[^\"]*\")[^>]*>([\s\S]*?)<\/section>/gis)];
  const parts = tagsSections.length ? tagsSections.map(m => m[1]).join(' ') : html;
  const matches = [...parts.matchAll(/<a[^>]*>(.*?)<\/a>/gis)];
...
function patchPostHtml(html, postTags) {
  let out = html;
  ...
  let tagIndex = 0;
  out = out.replace(/<a([^>]*class="tag"[^>]*)href="[^"]*"([^>]*)>(.*?)<\/a>/gis, (full, before, after, inner) => {
    const visible = strip(inner);
    const normalized = normalizeTag(visible || postTags[tagIndex] || '');
...
function renderTagPages(tagMap, posts) {
...
<body>
  <div class="wrap">
    ...
    <main>
      <section class="hero">
        <p class="eyebrow">Nube clickeable real</p>
  ...
  fs.writeFileSync(path.join(TAGS_DIR, 'index.html'), indexHtml, 'utf8');

  for (const tag of tags) {
    const tagPage = `<!doctype html>
<html lang="es">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
...
<body>
  <div class="wrap">
    ...
    fs.writeFileSync(path.join(TAGS_DIR, `${tag.slug}.html`), tagPage, 'utf8');
  }
}
